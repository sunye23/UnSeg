...
def main(net, train_datasets, valid_datasets, args):
    ...
    valid_im_gt_list = get_im_gt_name_dict_cityscapes(valid_datasets, flag="valid")
...
def evaluate(args, net, valid_dataloaders, visualize=False):
    ...
    train_json_path = os.path.join(root, 'cityscapes/gtFine/cityscapes_panoptic_train.json')
    with open(train_json_path, 'r') as file:
        train_datasets = json.load(file)
    city_annotations = train_datasets['annotations']
    image_dict = {}
    for data in city_annotations:
        image_dict[data['image_id']] = data
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            img_name_split = img_name[0].rsplit('_leftImg8bit', 1)[0]
            folder_name = img_name_split.split('_')[0]
            segments_info = image_dict[img_name_split]['segments_info']
            if torch.cuda.is_available():
            ...
            if args.infer_mode == 'single':
                ...
                for segment_info in segments_info:
                    ...
                    if not segment_info["iscrowd"] and class_id >= 24:
                        id_mask = gt_temp == segment_info["id"]
                        label_mask[id_mask] = 1.0
            ...
            if torch.sum(label_mask) != 0:
                with torch.no_grad():
                    masks_hq = net(input=batched_input)
                masks_hq = F.interpolate(masks_hq, size=pad_image.shape[-2:], mode='bilinear')
                ...
                try:
                    ...
                    pass
                save_path = os.path.join(full_save_dir, img_name[0] + '.png')
