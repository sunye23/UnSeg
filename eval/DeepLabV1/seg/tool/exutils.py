...
def get_data_from_chunk_city(chunk, args):
    ...
    for i, pieces in enumerate(chunk):
        piece = pieces.replace('.png', '').strip()
        img_names.append(piece)
        flip_p = np.random.uniform(0, 1)
        img_piece_path = os.path.join(img_path, piece+'.png')

        img_temp = cv2.imread(img_piece_path)
...
def get_data_from_chunk(chunk, args):
    ...
    target_dir = './image_aug/'
    target_dir_label = './label_aug/'
    ...
    for i, pieces in enumerate(chunk):
        ...
        # image_path = os.path.join(img_path, piece + '.jpg')
        # label_path = os.path.join(args.seg_pgt_path, piece + '.png')
        # img_target_path = os.path.join(target_dir, piece + '.jpg')
        # label_target_path = os.path.join(target_dir_label, piece + '.png')
        # if not os.path.exists(image_path):
        #     print("image_path missed = {}".format(image_path))
        #     continue
        # else:
        #     shutil.move(image_path, img_target_path)
        # if not os.path.exists(label_path):
        #     print("label_path missed = {}".format(label_path))
        #     continue
        # else:
        #     shutil.move(label_path, label_target_path)
        # continue
        img_piece_path = os.path.join(img_path, piece+'.jpg')
